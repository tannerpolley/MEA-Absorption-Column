...
DEFAULT_EPCSAFT_DATASET_NAME = os.environ.get(
    "MEA_EPCSAFT_DATASET_NAME", "MEA_reactive_epcsaft_2026_09_03"
)
...
def _resolve_epcsaft_dataset_path() -> Path:
    ...
    if override:
        ...
        if not override_path.is_dir():
            raise FileNotFoundError(f"Requested ePC-SAFT dataset directory does not exist: {override_path}")
...
class EpcsaftLiquidTransportState:
    ...
    molar_density_derivatives_mol_per_m3: np.ndarray
...
def epcsaft_liquid_transport_state(T, P, composition) -> EpcsaftLiquidTransportState:
    ...
    return EpcsaftLiquidTransportState(
        ...
        molar_density_derivatives_mol_per_m3=np.asarray(
            block["molar_density_derivatives_mol_per_m3"], dtype=float
        ),
