...
_EXPECTED_REACTION_SHA256 = "1989f3e6c8fa567a019dcdbceb4bbcf26d9ca48aec3f640dad1134bdd1fd4e7c"
...
def validate_reactive_bundle(dataset_text: str) -> dict:
    ...
    if any(bundle.get(key) != value for key, value in _EXPECTED_BUNDLE.items()):
        raise RuntimeError("Reactive ePC-SAFT bundle identity does not match the accepted bundle")
    expected = {
    ...
    if hashlib.sha256((dataset / "reaction-system.json").read_bytes()).hexdigest() != _EXPECTED_REACTION_SHA256:
        raise RuntimeError("Reactive ePC-SAFT reaction system identity does not match the accepted bundle")
