...
def single_shoot_solve(Y_a_scaled, Y_b_scaled, z, parameters, settings=None):
    ...
    if shoot:
        def shooter(X):
            ...
            Y_scaled, integrated_z, ivp_success, ivp_message = integrater(rhs, Y_a_scaled, z, args=parameters)
            if not ivp_success or len(integrated_z) != len(z):
                raise RuntimeError(f"Shooting trial integration failed: {ivp_message}")
            _raise_if_timed_out(settings)
    ...
    Y_scaled, integrated_z, ivp_success, ivp_message = integrater(rhs, Y_a_scaled, z, args=parameters)
    success = bool(success and ivp_success and len(integrated_z) == len(z))
    message = f"Shooting root: {message}; final integration: {ivp_message}"

    return Y_scaled, integrated_z, 'Single Shooting Method', success, message
