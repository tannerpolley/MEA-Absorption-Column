...
from src.mea_absorption_column.BVP.ABS_Column import abs_column
import numpy as np
...
def multiple_shoot_solve(inputs, guesses, df_param, scales):
    ...
    if shoot:
        ...
        def shooter(X, inputs, df_param, scales):
            ...
            for i in range(n_segments):
                ...
                if i < n_segments-1:
                    ...
                    eq1 = Y_i_scaled[0, -1] - Fl_CO2_i / scales[0]
                    eq2 = Y_i_scaled[1, -1] - Fl_H2O_i / scales[1]
                    eq3 = Y_i_scaled[4, -1] - Tl_i / scales[4]

                else:
                    eq1 = Y_i_scaled[0, -1] - Fl_CO2_z / scales[0]
                    eq2 = Y_i_scaled[1, -1] - Fl_H2O_z / scales[1]
                    eq3 = Y_i_scaled[4, -1] - Tl_z / scales[4]
        ...
        options = {'fatol': .05, 'maxiter': 50, 'line_search': 'armijo', 'disp': display, }
        # options = {}

        root_output = root(shooter, Y_guess, args=(inputs, df_param, scales), method=method, options=options)
