...
def run_model(df,
    ...
    stack_spec = build_bed_stack_spec(
        ...
        single_bed_height_m=case_metadata["single_bed_height_m"] if use_staged_beds else H,
...
Run #{run + 1:03d}:
    ...
    if method == "scipy-bvp" and (
        use_staged_beds
        or solver_settings_for_run.get("transform_mode") == "case_bounded_flow_pressure"
    ):
...
''')
    ...
    if return_details:
        ...
        result = {
            ...
            'solver_jacobian_calls': solver_diagnostics.get('solver_jacobian_calls'),
