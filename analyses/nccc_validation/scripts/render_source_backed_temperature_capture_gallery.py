...
DEFAULT_FIGURE_DIR = ANALYSIS / "results" / "final" / "figures" / "source_backed_1c_6c_temperature_capture_profiles"
DEFAULT_METRICS = ANALYSIS / "results" / "final" / "tables" / "source_backed_1c_6c_temperature_capture_profile_metrics.csv"
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render source-backed NCCC 2017 1C-6C temperature/capture validation PNGs."
    )
...
def _plot_case(
    ...
    ax.set_title(f"NCCC 2017 {case_id} source-backed profiles")
...
def _write_contact_sheet(plot_paths: list[Path], figure_dir: Path) -> Path:
    ...
    contact_sheet = figure_dir / "source_backed_1c_6c_temperature_capture_contact_sheet.png"
