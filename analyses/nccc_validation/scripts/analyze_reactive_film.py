...
def _retained_state() -> tuple[pd.Series, np.ndarray, np.ndarray]:
    source = _load_profile().iloc[-1]
...
def _run(mesh_points: int, initial_flux_factor: float) -> tuple[dict[str, object], pd.DataFrame]:
    source, bulk, diffusivities = _retained_state()
...
def main() -> None:
    ...
    cases = [(21, 1.0)] if not args.full else [(mesh, factor) for mesh in (21, 42) for factor in (0.5, 1.0, 2.0)]
    retained = {}
    if args.resume and RUN_TABLE.exists():
        existing = pd.read_csv(RUN_TABLE)
        retained = {
            (int(row.mesh_points), float(row.initial_flux_factor)): row.to_dict()
            for _, row in existing.loc[existing.outcome.eq("evaluated")].iterrows()
        }
    rows: list[dict[str, object]] = []
    reference_profile = pd.DataFrame()
    for mesh, factor in cases:
        if (mesh, factor) in retained:
            rows.append(retained[(mesh, factor)])
            continue
        try:
            row, profile = _run(mesh, factor)
            if mesh == max(case[0] for case in cases) and factor == 1.0:
    ...
    flux_spread = float(evaluated.interface_CO2_flux_mol_m2_s.max() / evaluated.interface_CO2_flux_mol_m2_s.min() - 1.0) if len(evaluated) else math.nan
    wheel = _wheel_path()
    summary = {
        ...
        "failed_rows": table.loc[~table.outcome.eq("evaluated"), ["mesh_points", "initial_flux_factor", "outcome", "diagnostic"]].to_dict(orient="records"),
        "claim_boundary": "Single retained Case 3C bulk state and effective-Fick Stage A only; no column-wide, WWC, reversible-kinetics, or Maxwell-Stefan validation claim.",
    }
