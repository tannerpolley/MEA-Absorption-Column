...
def base_row(source: dict[str, str], profile: dict[str, str] | None, source_table_sha: str, profile_sha: str) -> dict[str, Any]:
    row: dict[str, Any] = {
        ...
        "apparent_inputs_json": "", "apparent_input_units_json": json.dumps({"CO2": "mol s^-1", "MEA": "mol s^-1", "H2O": "mol s^-1", "normalized_basis": "mol mol^-1 analytical MEA"}, sort_keys=True),
        "apparent_input_reporting_intervals_json": "", "apparent_input_hashes_json": json.dumps({"issue33_table": source_table_sha, "retained_profile": profile_sha}, sort_keys=True),
        "temperature_K": "", "pressure_Pa": "", "temperature_reporting_interval": "", "pressure_reporting_interval": "", "packet_bound_candidate": "false", "packet_bound_status": "not_attempted", "packet_failure_code": "", "packet_failure_diagnostic": "",
        ...
        "inverse_apparent_totals_per_mol_N_json": "", "inverse_residuals_json": "", "inverse_max_abs_residual": "", "inverse_mapping_status": "not_attempted", "inverse_replay_status": "not_attempted", "forward_transform_status": "not_attempted", "forward_replay_status": "not_attempted", "forward_replay_max_abs_species_diff": "", "forward_replay_max_abs_density_diff": "",
    ...
    row.update(
        ...
        temperature_K=temperature_K, pressure_Pa=pressure_Pa, temperature_reporting_interval="retained profile value; experimental interval unavailable", pressure_reporting_interval="retained Case 3C input value; experimental interval unavailable",
    )
...
def apply_result(row: dict[str, Any], result: Any, replay: Any, run_id: str, command: str) -> None:
    ...
    row.update(
        ...
        forward_replay_status="identity_pass" if max(abs(a - b) for a, b in zip(x, replay_x, strict=True)) <= 1.0e-12 else "identity_failed", forward_replay_max_abs_species_diff=max(abs(a - b) for a, b in zip(x, replay_x, strict=True)), forward_replay_max_abs_density_diff=abs(density - float(replay.phases[0].molar_density_mol_m3)),
...
def main() -> int:
    ...
    try:
        ...
        run_id = "sha256:" + sha256_bytes(canonical({"bundle": bundle_meta, "source_table": source_table_sha, "profile": profile_sha, "generator": generator_sha, "selected_state": config["selected_state"]}))
        rows = []
        ...
        summary = {
            "claim_label": config["claim_label"], "analysis": "nccc_validation", "issue": 40, "rows": len(rows), "generator_sha256": generator_sha, "repository_commit": git_value("rev-parse", "HEAD"), "machine": platform.platform(), "workers": 1, "reproduction_command": command,
            ...
            "source_inputs": {"issue33_table_sha256": source_table_sha, "issue33_summary_sha256": sha256_path(ISSUE33_SUMMARY), "retained_profile_sha256": profile_sha},
            ...
            "gates": {"fixed_nine_species_order": list(SPECIES_IDS) == list(config["bundle"].get("species_order", SPECIES_IDS)), "single_liquid_only": all(row["phase_topology"] == "single_liquid_only" for row in rows), "vle_fugacity_equality_imposed": False, "position_1_exact_source_analytical_mol_L": config["selected_state"]["source_loaded_analytical_MEA_mol_L"], "position_1_exact_source_free_MEA_mol_L": config["selected_state"]["source_free_MEA_mol_L"], "position_1_source_basis_unresolved": True, "packet_evaluated_at_least_one_row": len(evaluated_rows) == 1, "analytical_and_elemental_residual_pass": max(float(row["inverse_max_abs_residual"]) for row in evaluated_rows) <= 1.0e-10, "mole_fraction_normalization_pass": max(float(row["mole_fraction_normalization_residual"]) for row in evaluated_rows) <= 1.0e-12, "charge_residual_pass": max(float(row["charge_residual"]) for row in evaluated_rows) <= 1.0e-10, "deterministic_replay_pass": all(row["forward_replay_status"] == "identity_pass" for row in evaluated_rows), "no_capture_inference": all(row["capture_inference_used"] == "false" for row in rows), "no_thermo_or_kinetic_fit": all(row["thermodynamic_or_kinetic_fit_performed"] == "false" for row in rows)},
...
Command: `{command}`<br>
Repository commit: `{summary['repository_commit']}`<br>
