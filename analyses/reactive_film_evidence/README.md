...
- Putta2016 Table 4 provides 20 exact aggregate AARD values across SDC, two WWC data groups, and a laminar-jet group; these compare models but do not expose row-level measured fluxes or uncertainty. **[verified]**
...
- Polat et al. (2023), DOI `10.1016/j.fluid.2022.113587`: Table 2, PDF p. 4 (repository PDF page index 4), 293-353 K. **[verified]**
...
- The Putta F1/F2 forms can drive a finite-rate prototype only after their standard-state mapping is made consistent with the thermodynamic provider; they cannot yet be validated from the retained aggregate AARD table. **[reference-backed]**
- The Luo/Snijder reduced scalar correlations and Polat self-diffusion fits can define labeled comparator ranges, but converting them into a multicomponent mobility matrix would add unsupported physics. **[reference-backed]**
- The central figure shows that Table 4 errors depend on both model basis and apparatus/data grouping; it is aggregate model-comparison evidence, not independent row-level film validation. **[verified]**

## Best next strategy

1. Acquire the exact Luo2015 supplementary file and retain all 227 rows with apparatus membership, units, state variables, and reported measurement accuracy. **[reference-backed]**
2. Acquire and classify Ramezani2021 and the Polat2023 supplement; use self-diffusion only as tracer evidence unless the source provides a defensible conversion or cross-correlation information. **[reference-backed]**
