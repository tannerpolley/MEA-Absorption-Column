...
@pytest.mark.parametrize("model", ["epcsaft_ionic", "epcsaft_electrolyte", "epcsaft_full_ionic"])
def test_ionic_equilibrium_aliases_route_to_bundle_backed_nine_species(model):
    _requires_reactive_epcsaft_dataset()
    Fl = [0.1529, 1.0, 7.911]
    expected = chemical_equilibrium_with_model(Fl, 313.15, model="epcsaft_reactive_nine")
    actual = chemical_equilibrium_with_model(Fl, 313.15, model=model)
...
def test_converted_and_rebased_nine_species_equilibrium_aliases_fail_closed(model):
    with pytest.raises(RuntimeError, match="converted/rebased nine-species"):
        chemical_equilibrium_with_model([0.1529, 1.0, 7.911], 313.15, model=model)


def test_tabulated_reactive_equilibrium_requires_bundle_identity(tmp_path, monkeypatch):
    path = tmp_path / "speciation.csv"
    path.write_text("status,temperature_k,loading\nevaluated,320,0.3\n", encoding="utf-8")
    monkeypatch.setenv("MEA_EPCSAFT_REACTIVE_TABLE", str(path))
    with pytest.raises(RuntimeError, match="sidecar identity"):
        chemical_equilibrium_with_model(
            [0.3, 1.0, 8.0], 320.0,
            model="epcsaft_reactive_nine_tabulated", diagnostics={},
        )
