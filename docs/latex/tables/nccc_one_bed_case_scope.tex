\begin{table}[pos=htbp]
  \centering
  \scriptsize
  \caption{NCCC one-bed operating inputs and observed CO$_2$ capture without intercooling: 2014 cases \cite{Morgan2018a} and 2017 cases \cite{Morgan2020}. Gas CO$_2$ fractions are on a dry basis; MEA mass fractions refer to CO$_2$-free solvent.}
  \label{tab:nccc-one-bed-scope}
  \begin{threeparttable}
  \setlength{\tabcolsep}{2.5pt}
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}ll
    S[table-format=5.0]
    S[table-format=4.0]
    S[table-format=2.1,table-space-text-post=\tnote{$\dagger$}]
    S[table-format=2.1]
    S[table-format=3.1]
    S[table-format=1.2]
    S[table-format=1.2]
    S[table-format=1.3]
    S[table-format=2.1]@{}}
    \toprule
    Year & Case &
    \multicolumn{1}{c}{\shortstack{Lean\\flow\\(\unit{kg\,h^{-1}})}} &
    \multicolumn{1}{c}{\shortstack{Gas\\flow\\(\unit{kg\,h^{-1}})}} &
    \multicolumn{1}{c}{\shortstack{Lean\\temp.\\(\unit{\degreeCelsius})}} &
    \multicolumn{1}{c}{\shortstack{Gas\\temp.\\(\unit{\degreeCelsius})}} &
    \multicolumn{1}{c}{\shortstack{Pressure\\(\unit{kPa})}} &
    \multicolumn{1}{c}{\shortstack{Lean\\loading}} &
    \multicolumn{1}{c}{\shortstack{MEA\\mass frac.}} &
    \multicolumn{1}{c}{\shortstack{Inlet\\$y_{\ce{CO2}}$}} &
    \multicolumn{1}{c}{\shortstack{Capture\\(\unit{\percent})}} \\
    \midrule
    2014 & K18 & 6804  & 2271 & 42.2 & 46.1 & 109.2 & 0.14 & 0.30 & 0.102 & 92.8 \\
    2014 & K19 & 11793 & 1440 & 40.9 & 46.2 & 108.0 & 0.18 & 0.28 & 0.110 & 98.2 \\
    2014 & K20 & 3175  & 1324 & 45.3 & 46.1 & 108.1 & 0.07 & 0.28 & 0.110 & 95.5 \\
    2017 & 1C & 6185 & 1997 & 45.0\tnote{$\dagger$} & 43.3 & 109.1 & 0.15 & 0.30 & 0.077 & 97.1 \\
    2017 & 2C & 7765 & 2499 & 45.0\tnote{$\dagger$} & 43.1 & 109.6 & 0.20 & 0.30 & 0.077 & 92.3 \\
    2017 & 3C & 7517 & 2013 & 45.0\tnote{$\dagger$} & 43.6 & 109.5 & 0.25 & 0.30 & 0.093 & 89.5 \\
    2017 & 4C & 6160 & 1500 & 44.9 & 43.3 & 107.7 & 0.25 & 0.30 & 0.108 & 88.9 \\
    2017 & 5C & 5237 & 1498 & 45.0 & 43.3 & 108.4 & 0.26 & 0.30 & 0.077 & 86.4 \\
    2017 & 6C & 7665 & 2700 & 44.8 & 43.4 & 109.7 & 0.31 & 0.31 & 0.077 & 60.2 \\
    2017 & 7C & 5414 & 1000 & 44.7 & 43.3 & 107.3 & 0.34 & 0.29 & 0.100 & 76.4 \\
    \bottomrule
  \end{tabular*}
  \begin{tablenotes}[flushleft]
    \footnotesize
    \item[$\dagger$] Lean solvent inlet temperature was blank in the source table and was set to \qty{45.0}{\degreeCelsius} for the model input.
  \end{tablenotes}
  \end{threeparttable}
\end{table}
