    ...
    \subsection{Accuracy-Credibility Screens}
        ...
        Figure~\ref{fig:error-regime-capture-error} plots capture error against three operating descriptors: liquid-to-gas ratio, lean loading, and inlet CO$_2$ mole fraction. The corrected campaign inputs change the interpretation from a case-index trend to an operating-regime trend: both thermodynamic lanes remain within ten percentage points across the seven one-bed C cases, and the largest capture misses are concentrated at lower-capture operating points. This gives the benchmark a practical role: it shows where the present equation set is already useful as a predictive approximation and where future calibration or submodel refinement should be concentrated.

        \begin{figure}
            \centering
            \includegraphics[width=0.92\textwidth]{figures/error-regime-capture-error.pdf}
            \caption{Capture-error regimes for the corrected one-bed C-case campaign inputs. Both thermodynamic lanes converge for all seven cases; capture errors remain below ten percentage points and cluster around lower-capture operating points rather than a monotonic endpoint artifact.}
    ...
    \subsection{Solver-Method Contrast}
        The solver comparison is reported as a method-behavior benchmark rather than as a claim that every method is equally useful for every absorber case. Figure~\ref{fig:method-case-solver-contrast} contrasts a favorable SRP-style one-bed case with the NCCC 3C thermal-pinch case. The shooting and finite-difference paths can produce useful comparison results under favorable conditions, while the collocation path is the more reliable reference workflow for the NCCC temperature-profile case.

        Table~\ref{tab:method-case-contrast} gives the corresponding method results. The SRP-style case is included because it shows the shooting and finite-difference implementations on a smoother one-bed operating point. The NCCC 3C case is more discriminating because the measured profile contains a sharper temperature feature, and the initial-value character of shooting and the algebraic stiffness of the finite-difference residual are less forgiving than the collocation BVP formulation.
        ...
        \begin{figure}
            ...
            \caption{Runtime and method comparison for representative one-bed cases. The SRP-style case shows shooting and finite difference on a smoother operating point; the NCCC 3C case shows why collocation BVP solution is retained as the reference workflow for the validation set.}
