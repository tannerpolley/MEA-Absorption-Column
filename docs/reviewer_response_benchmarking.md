...
- `epcsaft_ionic`: ePC-SAFT fugacity-coefficient sensitivity using the external `C:\Users\Tanner\Documents\git\ePC-SAFT` package read-only and the vendored `MEA_CO2_H2O_ionic_fit` dataset.
...
- Multi-bed and intercooled NCCC rows can be routed through a staged collocation BVP solver. The staged solver uses one packed-bed BVP per bed, enforces inter-bed vapor continuity, and applies liquid continuity or liquid enthalpy reset between beds.

## Publication Limits

The ePC-SAFT comparison isolates only the CO2 fugacity driving force. Chemistry, enhancement factor, transport properties, hydraulic correlations, and balances are intentionally unchanged for the first comparison. This must be described as a thermodynamic sensitivity study, not as a complete electrolyte-reactive ePC-SAFT absorber model.
...
uv run python -m mea_absorption_column.benchmark --methods single scipy-bvp --thermo-models ideal_henry epcsaft_ionic
uv run python -m mea_absorption_column.benchmark --methods scipy-bvp --thermo-models ideal_henry epcsaft_ionic --staged-beds auto --output-dir analyses\nccc_validation\results\runs\intercooled_benchmark
