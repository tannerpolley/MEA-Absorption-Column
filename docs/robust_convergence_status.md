...
- The staged collocation BVP path now supports a case-bounded flow/pressure transform. An experimental `thermal_state_mode=temperature` path solves staged beds directly in liquid/vapor temperature variables and computes enthalpy algebraically inside the column model.
...
- The ePC-SAFT thermodynamic adapter exposes `epcsaft_fugacity_blend` / `--epcsaft-fugacity-blend` so staged runs can diagnose Henry-to-ePC-SAFT branch changes without changing the chemistry, transport, or balance submodels.
...
- Fugacity evaluation has a guarded path for both `ideal_henry` and `epcsaft_ionic`; invalid ePC-SAFT states are recorded and converted to finite fallback fugacities instead of uncaught exceptions.
- Collocation BVP benchmark settings now expose mesh size, tolerances, max nodes, and the opt-in hand-built finite Jacobian. The hand-built central-difference Jacobian is no longer the default because it is too slow for stacked beds.
...
- Henry C-case collocation BVP benchmark: all seven C cases converged in the current campaign-input validation run.
- ePC-SAFT C-case collocation BVP benchmark: all seven C cases converged in the current campaign-input validation run.
...
- K1 continuation with optional seed/ramp stages reaches a required full-intercooler stage with capture near the NCCC value and near-zero boundary residual. K2 can preserve the high-capture branch at partial ePC-SAFT fugacity blends in older smoke runs, but full multi-bed validation remains outside the current one-bed C-case evidence set.
...
The next development step should focus on a continuation path that preserves a plausible capture/temperature profile across stages: one-bed target profile, staged beds without intercooler reset, gradual intercooler-strength ramp, then bounded Henry-to-ePC-SAFT fugacity blending with branch checks at each thermodynamic endpoint. The temperature-state path also needs residual scaling work before it can replace enthalpy-state solves for the NCCC benchmark.
