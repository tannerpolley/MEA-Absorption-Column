...
- The neutral ePC-SAFT thermodynamic adapter now exposes `epcsaft_fugacity_blend` / `--epcsaft-fugacity-blend` so staged runs can diagnose Henry-to-ePC-SAFT branch changes without changing the chemistry, transport, or balance submodels.
...
- K1 continuation with optional seed/ramp stages reaches a required full-intercooler stage with capture near the NCCC value and near-zero boundary residual. K2 can preserve the high-capture branch at partial neutral ePC-SAFT fugacity blends of 0.50 and 0.75, and a 0.90 blend reaches the capture/residual target but times out. The full blend value of 1.00 still collapses to the zero-capture branch, so K2 remains unresolved for the full neutral ePC-SAFT endpoint.
