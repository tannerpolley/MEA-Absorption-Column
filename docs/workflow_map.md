...
| `analyses/nccc_validation/` | Analysis | Canonical reviewer-response validation workflow for C cases, staged/intercooled NCCC cases, solver comparisons, dense profile CSVs, and manuscript figures. | Disposable runs under `results/runs/`; curated evidence under `results/final/`. |
