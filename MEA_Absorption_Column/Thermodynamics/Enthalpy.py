...
def liquid_enthalpy(Tl):
    ...
    Tl = float(Tl)
    Hl_CO2 = -84000
    Hl_MEA = MWs_l[1]*sum([coefficients['MEA'][i]*(Tl - Tr)**(i+1) for i in range(len(coefficients['MEA']))]) - dh_vap_MEA
    Hl_H2O = MWs_l[2] * sum([coefficients['H2O'][i] * (Tl - Tr) ** (i + 1) for i in range(len(coefficients['H2O']))]) - dh_vap_H2O
