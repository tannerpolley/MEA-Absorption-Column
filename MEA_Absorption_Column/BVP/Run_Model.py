...
def run_model(df,
    ...
    Fv_CO2_b_guess = (1 - (CO2_cap_guess / 100)) * Fv_CO2_a
    Fv_H2O_b_guess = (1 - (H2O_cap_guess / 100)) * Fv_H2O_a
    Tv_b_guess = 335.

    Fl_CO2_a_guess = Fl_CO2_b + (Fv_CO2_a - Fv_CO2_b_guess)
    Fl_H2O_a_guess = Fl_H2O_b + (Fv_H2O_a - Fv_H2O_b_guess)
    Tl_a_guess = 325.
    ...
    parameters = scales, const_flow, H, A, packing
...
Run #{run + 1:03d}:
    ...
    # CO2
    Fl_CO2_a_sim, Fl_CO2_b_sim = Y[0, 0], Y[0, -1]
    Fv_CO2_a_sim, Fv_CO2_b_sim = Y[2, 0], Y[2, -1]

    # H2O
    Fl_H2O_a_sim, Fl_H2O_b_sim = Y[1, 0], Y[1, -1]
    Fv_H2O_a_sim, Fv_H2O_b_sim = Y[3, 0], Y[3, -1]

    # Temperature
    Tl_a_sim, Tl_b_sim = Y[4, 0], Y[4, -1]
    Tv_a_sim, Tv_b_sim = Y[5, 0], Y[5, -1]

    # Computes the relative error between the solution that the shooter found to the
    # actual inlet concentration for the relevant liquid species
    ...
    # Prints out relevant info such as simulation time, relative errors, CO2% captured,
    # if max iterations were reached and number of Nan's counted
