...
from MEA_Absorption_Column.Properties.Thermophysical_Properties import density, viscosity, surface_tension, diffusivity, heat_capacity, thermal_conductivity, henrys_law, enthalpy
from MEA_Absorption_Column.Properties.Transport_Properties import velocity, holdup, interfacial_area, flooding_fraction

from MEA_Absorption_Column.Thermodynamics.Driving_Force import driving_force
...
from MEA_Absorption_Column.Transport.Mass_Transfer_Coeff import mass_transfer_coeff
from MEA_Absorption_Column.Transport.Heat_Transfer_Coeff import heat_transfer_coeff
...
def abs_column(zi, Y_scaled, parameters, run_type='simulating', column_names=False):
    ...
    # region --- Enthalpy
    Hl_CO2, Hl_MEA, Hl_H2O = enthalpy(Tl, x, phase='liquid')  # J/mol
    Hv_CO2, Hv_H2O, Hv_N2, Hv_O2 = enthalpy(Tv, y, phase='vapor')  # J/mol
    # endregion
    ...
    DF_CO2, DF_H2O, CO2, H2O = driving_force(x, y, x_true, Cl_true, Tl, Tv, alpha, H_CO2_mix, P)
    ...
    E, Psi, KH, enhance_factor = enhancement_factor(Tl, Cl_true, y[0], P, H_CO2_mix, kl_CO2, kv_CO2,
    ...
    # Liquid
    Nl_CO2 = kv_CO2 * DF_CO2 * a_eA * KH  # mol/(s*m)
    Nl_H2O = kv_H2O * DF_H2O * a_eA  # mol/(s*m)

    # Vapor
    Nv_CO2 = -Nl_CO2  # mol/(s*m)
    Nv_H2O = -Nl_H2O  # mol/(s*m)
    # endregion

    # region --- Enthalpy Transfer

    Hl_CO2_trn, Hl_H2O_trn = Nl_CO2 * Hl_CO2, Nl_H2O * Hl_H2O  # J/(s*m)
    Hv_CO2_trn, Hv_H2O_trn = Nv_CO2 * Hv_CO2, Nv_H2O * Hv_H2O  # J/(s*m)

    Hl_trn = Hl_CO2_trn + Hl_H2O_trn  # J/(s*m)
    Hv_trn = Hv_CO2_trn + Hv_H2O_trn  # J/(s*m)
    Hl_trn = -Hv_trn  # J/(s*m)

    # endregion

    # region --- Heat Transfer
    Ql_trn = UT * (Tv - Tl) * a_eA # J/(s*m) =  J/(s*K*m) * K * m or W/(K*m) * K
    Qv_trn = -UT * (Tv - Tl) * a_eA # J/(s*m) =  J/(s*K*m) * K * m or W/(K*m) * K
    ...
    if run_type == 'simulating':
        ...
        DF_CO2, Pv_CO2, Pl_CO2, H_CO2_mix = CO2
        DF_H2O, Pv_H2O, Pl_H2O, Psat_H2O = H2O
        k2, Cl_MEA_true, Dl_CO2, kl_CO2, Ha, E, KH, Psi = enhance_factor
        ...
        output_dict = {
            ...
            'qv': [Tv, Hv_CO2, Hv_H2O, Hv_N2, Hv_O2, Hv, Hv_CO2_trn, Hv_H2O_trn, Hv_trn, Qv_trn, dHvf_dz,
                   Hvf],
            'CO2': [Nl_CO2, Nv_CO2, kv_CO2, a_eA, DF_CO2, Pv_CO2, Pl_CO2, Psi, H_CO2_mix],
            'H2O': [Nl_H2O, Nv_H2O, kv_H2O, a_eA, DF_H2O, Pv_H2O, Pl_H2O, Psat_H2O],
