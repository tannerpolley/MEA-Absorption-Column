...
def liquid_enthalpy(Tl, Tv, x):

    ΔH_abs = -83999.8249763614
    Hv_CO2 = vapor_enthalpy(Tv, x)[0][0]
    b, c, d = -5876, -8.598, -.012
    Hl_CO2 = Hv_CO2 - R*(-b + c*Tl + d*Tl**2) + ΔH_abs
    ...
    Hl_MEA = Σ_Cp(Tl, 'MEA') - heat_of_vaporization(Tl, 'MEA')
    Hl_H2O = Σ_Cp(Tl, 'H2O') - heat_of_vaporization(Tl, 'H2O')
...
def enthalpy(Tl, Tv, x, y, phase='liquid'):
    if phase == 'liquid':
        return liquid_enthalpy(Tl, Tv, x)
    elif phase == 'vapor':
        return vapor_enthalpy(Tv, y)
    else:
