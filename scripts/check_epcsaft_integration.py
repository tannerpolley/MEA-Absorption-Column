...
IMPORT_PATTERN = re.compile(
    r"^\s*(?:from\s+epcsaft(?:\.[\w.]+)?\s+import\b|import\s+epcsaft\b)"
)
...
def _local_wheel_identity(direct_url: dict[str, Any] | None) -> dict[str, str] | None:
    ...
    if not wheel_path.is_file():
        raise RuntimeError(
            f"Installed ePC-SAFT wheel source no longer exists: {wheel_path}"
        )
...
def _native_core_identity(dist) -> dict[str, str]:
    core_file = next(
        (
            file
            for file in (dist.files or ())
            if str(file).startswith("epcsaft/_core") and str(file).endswith(".so")
        ),
        None,
    )
    if core_file is None:
        raise RuntimeError(
            "Installed ePC-SAFT distribution has no native _core artifact"
        )
    core_path = Path(dist.locate_file(core_file)).resolve()
    return {
...
def resolve_epcsaft(contract: dict[str, Any]) -> dict[str, Any]:
    thermo_models = importlib.import_module(
        "mea_absorption_column.Thermodynamics.thermo_models"
    )
    ...
    if direct_url and "vcs_info" in direct_url:
        ...
        source_kind = (
            "local_file" if str(direct_url["url"]).startswith("file:") else "direct_url"
        )
        source_detail = json.dumps(direct_url, sort_keys=True)
    ...
    resolved.update(_native_core_identity(dist))
    return resolved
...
def scan_direct_imports(contract: dict[str, Any]) -> list[str]:
    allowlist = {
        entry.replace("\\", "/")
        for entry in contract.get("allowed_direct_import_paths", [])
    }
    private_exceptions = {
        entry.replace("\\", "/")
        for entry in contract.get("private_import_exception_paths", [])
    }
    ...
    for path in _iter_python_files(contract.get("scan_roots", [])):
        ...
        try:
            ...
            failures.append(
                f"{rel}:{exc.lineno}: could not parse Python for import scanning"
            )
            continue
        ...
        if rel not in private_exceptions:
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Import):
                    ...
                    _is_import_module_call(node)
                    or _is_spec_from_file_location_call(node)
                ):
                    candidate = _string_arg(node)
                    if candidate and any(
                        candidate.startswith(prefix) for prefix in forbidden_prefixes
                    ):
                        module_name = candidate

                if module_name is not None:
                    failures.append(
                        f"{rel}:{line_no}: uses forbidden private epcsaft module {module_name}"
                    )
                    break
        ...
        for line_no, line in enumerate(text.splitlines(), start=1):
            if IMPORT_PATTERN.search(line):
                failures.append(
                    f"{rel}:{line_no}: direct epcsaft import outside allowlist"
                )
    return failures
...
def run_smoke(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    if not dataset_path.exists():
        raise RuntimeError(
            f"Expected vendored or configured dataset path to exist: {dataset_path}"
        )
    composition = [1.0, 20.0, 70.0, 3.0, 2.0, 0.5, 0.25, 0.5, 0.5]
    total = sum(composition)
    state = thermo_models.epcsaft_liquid_transport_state(
        318.15,
        109500.0,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Check MEA-Absorption-Column ePC-SAFT integration contract."
    )
    parser.add_argument(
        "--mode",
        choices=("stable", "dev", "final"),
        default=None,
        help="Select the package source lane.",
    )
    parser.add_argument(
        "--self-only", action="store_true", help="Skip the repo-specific smoke step."
    )
    ...
    if mode == "final":
        ...
        if resolved.get("core_sha256") != identity["core_sha256"]:
            errors.append(
                f"Resolved ePC-SAFT core SHA-256 {resolved.get('core_sha256')!r} "
                f"does not match recorded identity {identity['core_sha256']!r}."
            )
    ...
    try:
        ...
        errors.append(
            f"Could not compare resolved epcsaft version {resolved['version']!r}."
        )
    ...
    for module_name, symbols in contract.get(
        "required_public_module_symbols", {}
    ).items():
    ...
    print(f"epcsaft core path: {resolved['core_path']}")
    print(f"epcsaft core SHA-256: {resolved['core_sha256']}")
    if mode == "final":
        engine_commit = contract["final_identity"].get("engine_commit")
        print(
            f"Engine commit: {engine_commit}"
            if engine_commit
            else "Engine commit: unresolved provenance (runtime identity verified separately)"
        )
    if smoke_payload is not None:
        ...
        print(f"artifact fingerprint: {smoke_payload['artifact_fingerprint']}")
        print(f"CO2 fugacity (Pa): {smoke_payload['co2_fugacity_pa']}")
        print(
